...
class StyleReg {
     ...
     */
    public globals<T extends Context>(styles: Record<string, object | ((context: T) => object)>) {
     ...
     */
    public css<T extends Context>(styles: Record<string, object | ((context: T) => object)>): Record<string, string> {
 ...
 */
    public resolveGlobals(): string {
        ...
        this.globalStyles.forEach((declaration, selector) => {
            ...
            const resolvedDeclaration = typeof declaration === 'function'
            ...
            const result = postcss([postcssNested]).process(wrappedStyles, { parser: postcssJs });
